...
// صنف يمثل الكتاب ويحتوي على معلوماته الأساسية
class Book {
    ...
    public String getDetails() {
        return title + " - " + author + " (ISBN: " + isbn + ") | " 
               + (isAvailable ? "متاح" : "معار");
    }
}

// صنف يمثل المستعير ويحفظ قائمة الكتب المستعارة
class Borrower {
    ...
    public String returnBook(Book book) {
        ...
        return " " + name + " لم يقم باستعارة هذا الكتاب.";
    }
}

// صنف يمثل عملية الإعارة ويتابع تاريخ الإعارة والاسترجاع
class BorrowingProcess {
    ...
    public void showBorrowingDetails() {
        ...
        System.out.println("وقت الإعارة: " + borrowDate 
                           + " | تاريخ الاسترجاع: " + (returnDate != null ? returnDate : "قيد الإعارة"));
    }
}

// صنف يمثل المكتبة ويدير إضافة الكتب والمستعيرين وتنفيذ الإعارات
class Library {
    ...
    public void addBook(Book book) {
        books.add(book);
        System.out.println("بنجاح تمت إضافة الكتاب '" + book.getDetails() + "'.");
    }

    public void addBorrower(Borrower borrower) {
        borrowers.add(borrower);
        System.out.println("بنجاح تمت إضافة المستعير '" + borrower + "'.");
    }
...
public class LibraryManagement {
    public static void main(String[] args) {
        ...
        Borrower borrower1 = new Borrower("abdelmalek", "S2025");
...
while (true) {
    ...
    scanner.nextLine();  // لتجنب مشاكل الإدخال

    if (choice == 1) {
        System.out.print("أدخل عنوان الكتاب: ");
        String title = scanner.nextLine();
        library.borrowBook(borrower1, 
            book1.getDetails().contains(title) ? book1 : book2);
    
    } else if (choice == 2) {
        System.out.print("أدخل عنوان الكتاب: ");
        String title = scanner.nextLine();
        library.returnBook(borrower1, 
            book1.getDetails().contains(title) ? book1 : book2);
